...
describe("App - generation d'equations", () => {
  let randomSpy;
  let workerStub;
  let alertSpy; // <-- Déclaration de la variable pour le spy de l'alerte
  ...
  beforeEach(() => {
    ...
    // <-- Remplacement de l'ancien vi.stubGlobal("alert", vi.fn()) par ton spy persistant :
    alertSpy = vi.fn();
  ...
  afterEach(() => {
    ...
    vi.unstubAllGlobals(); // <-- Nettoie proprement le stub d'alert et de Worker
    vi.restoreAllMocks();
  });
